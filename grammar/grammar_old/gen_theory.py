...
from grammar.parser import read_model_txt, print_sequence
...
def generate_theory(grammar_masker: GrammarMasker, 
    ...
    #print(f"Search Space: {search_space:.1e}")
    return sequence, valid_token_list
...
def gen(shard_id, num_shards): 
    ...
    for i in range(shard_id*num_shards, (shard_id+1)*num_shards):
        ...
        sequence, valid_token_list = generate_theory(grammar_masker, prompt=prompt, print_tokens=False, seed=i)
...
if __name__ == "__main__":
    main(TOTAL = 1000000, NUM_PROC = 10, save_path = "dataset/pretrain_data_1M")
